...
- Narrow (< 1024px): the pane is a sheet over the list opened by a provenance link, with a header (document title + tabs), a `Close` control ≥ 44px (x icon from the set + the word), and a footer jump link "Back to {field}" (arrow-left icon + text, underlined like every jump link); `Esc` and `Close` return focus to the link that opened it. Body scroll is locked while the sheet is open. While a draft exists, a one-line entry at the head of the field list — "Clarification draft · 3 fields · Open" (text button, narrow only) — opens the sheet on the Clarification tab.
