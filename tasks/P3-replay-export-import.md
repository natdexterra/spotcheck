...
- **End.** When the last step has run the replay stays attached: playing false, `ended` true, the counter reads "41 / 41", the row shows `Restart` only. Persistence stays suspended (the sample must not overwrite the saved session).
...
`Play` / `Pause` is one button whose label changes; it keeps focus across the change. `Restart` moves focus to `Play`. Labels never wrap; on narrow (< 1024px) the row stacks: the text line, then the controls row (wrapping, left-aligned, 44px targets). The row leaves with `leave()`; the quiet line above it is unchanged (a replay is `live`, per P2's precedence rule).
...
- Controls: `Play sample session` → the row reads "Sample session · recorded 2026-09-01 · 0 / 26"; after `runFor(3000)` the counter advanced; `Pause` → the counter is frozen across `runFor(5000)`; `Next call` → exactly +1; `Restart` → "0 / 26", eleven "Not extracted" rows, focus on `Play`; run to the end → "26 / 26 · finished", only `Restart` visible.
