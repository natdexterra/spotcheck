...
| `src/components/FieldList.tsx` (modify) | the group heading (`field-list__group-heading`) renders only while the list holds more than one group (§ First load and settled rows); the grouping, the sort and the collapsed verified group are unchanged |
...
| `DESIGN.md` (modify) | § Interaction states, one line: a modal dialog counts as a screen of its own — its single primary is the large primary (one per dialog); the compact local-primary rule stays a rule for rows and open editors. § Choice controls, one line: `<dialog>`, `<textarea>` and `<input type="file">` stay native — styled only through tokens (font, color, border, radius, focus ring), no re-drawn replacement. § Color adds one token `--scrim` (`--ink` at 32% alpha via `color-mix`, on the dark ground only) for the dialog's `::backdrop`; no blur, per § No shadows. Four further additions, same commit as the code they describe: **(a)** § Spacing, layout, one line: a group heading appears only when the field list holds more than one group; the first-load screen is eleven bare rows under the pane header (export 01). **(b)** § Copy grammar, one line: a settled row (`verified` with any resolution) keeps the `Agent: {rationale}` line and the searched chips: the evidence behind a decision stays visible after it; `docs/design/README.md` gets a note on row 11 that the export omits them and the rule wins. **(c)** A new section **Components**: one table with every shared primitive named in `AGENTS.md` (the Button variants and sizes, the Choice kinds, Badge, ProvenanceLink, the card, the disclosure, the inline editor, the sheet, and the dialog this task adds), and per primitive its CSS block name in `src/styles/components.css`, the React component that owns it (for a primitive that is a class convention without a component of its own, such as the card and the disclosure, the components that render it), its states, and its heights per breakpoint (the control-height tokens already in this file, cited, not retyped). `AGENTS.md` lists the dialog among primitives whose class names are "already in" the stylesheet, which is untrue until this task lands: the same commit that adds the dialog block corrects that sentence. **(d)** A new paragraph **Absent states** under § Interaction states: for each component that can be hidden or that hides parts of itself, the mechanism (`hidden` with the global guard from `AGENTS.md`, conditional rendering, or a modifier) and which parts disappear in which state: an open editor hides the value line, links, rationale and actions; the strip's export slot; the source pane after confirm; the Spec and Drawing tabs for a package without those documents; the dialog when closed. **(e)** A new paragraph **String sources** under § Copy grammar: which strings are app-authored (the copy rules apply: no em or en dash, middle-dot separators, plurals) and which are content rendered verbatim (document text from `data/` and from a user package, agent rationale, notes, drafts); the exclusion list of `src/copy.test.ts` mirrors this paragraph and a comment there points to it |
...
7. Actions: `Open package` (the dialog's primary, large — a dialog is a screen, per the § Interaction states line this task adds) · `Cancel` (text) · `Use the sample package` (text, only while a user package is open).
...
### First load and settled rows (owner's rulings, 2026-09-02)

- **One group, no heading.** `FieldList` renders a group heading only while more than one group is open. On first load every field is `empty`, so the list is the pane header and eleven bare rows, as export 01 draws it; the first heading appears with the first proposal, when a second group exists. Nothing else about grouping changes. Unit test: one group → no `heading` role inside the list beyond the pane title; two groups → two headings. The existing first-load e2e assertions (`layout.spec.ts`, `narrow.spec.ts`) gain the absence check.
- **Settled rows keep their evidence.** A row in `verified` (any resolution) keeps the `Agent: {rationale}` line and the searched chips exactly as production renders them today; export 11 omits them and is superseded on this point (the DESIGN.md row above, item (b)). No code change; the rule is written so the next task does not remove them.
...
## Leaving a session (owner's finding on the P3.1 preview, 2026-09-02)

A person who starts `Play sample session` and does not want to watch it through has no way back today: `Restart` replays from step 0, `Start over` exists only on the confirm screen, and a reload restores the saved session by design (P1.1). Two exits, both in this task because both touch the files this task already owns (`StatusStrip.tsx`, `ReplayControls.tsx`, `src/replay/controller.ts`, the expanded change-log header, the dialog):

1. **`Leave sample` in the replay row, in every state.** The row's controls become: playing `Pause · Leave sample`; paused `Play · Next step · Leave sample`; ended `Leave sample`; error `Leave sample`. `Restart` is removed everywhere (a 26-step recording is short; watching again is `Leave sample` → `Play sample session`). For an imported session the button reads `Leave session`; the `{label}` rule of P3.1 § Replay controls applies. Behaviour: `leave()` (the P3 detach sequence, one replay owner), which restores the pre-replay snapshot: the package that was current before the replay (§ Scope, the `controller.ts` row) and the saved live session, if there was one. Then two cases. **The snapshot was empty** (the person started the sample on a fresh page, the common case): `replaceState(createInitialState())`, the saved session key cleared, the strip back to `waiting` or `no-api` per the API state, the field list to the eleven bare rows, the change log to "No activity yet". **The snapshot held live work** (the person had real proposals before pressing `Play sample session`): that work is back on the page and nothing more happens — the strip reads `live` again, the log carries the restored entries; the replay's own entries are gone with the replay. In both cases focus goes to the control the person can use next (`Play sample session` on the fresh page; the first open row's primary action on the restored one), live region "Sample session closed" / "Imported session closed". No confirmation, because in neither case is anything of the person's lost — the reset applies only to a page that was empty before the replay. The control slot keeps its fixed width (the widest set is the paused one); below 600px the controls stack under the text as P3.1 ruled. Note for the builder: `src/replay/persistence.ts` keeps a fixed key (`spotcheck.session.v1`) and exports no clear function today; the key clearing named here and in § Persistence is new work in the `persistence.ts` row. The P3.1 rule "`Restart` focuses `Play`" is withdrawn; this section supersedes the controls column of the P3.1 § Replay controls table, and the P3.1 task file gets a one-line pointer to here in the same commit.
2. **`Start over` during a live session**, in the expanded change-log header beside `Play sample session` (every width), for the person whose agent went wrong. It discards real work, so it confirms through the same native `<dialog>` this task builds (title "Start over?", one line "This discards {N} entries and every value on the page.", `Start over` primary large, `Cancel` text; `Esc` cancels; focus returns to the button). On confirm: the same reset as above minus the replay detach; live region "Review cleared". The confirm screen's existing `Start over` (P3) keeps its behaviour and its place.

Tests: unit — the controls per state contain no `Restart` and always contain the leave button; `leave` + reset clears storage and returns the initial state; the live `Start over` opens the dialog and resets only on confirm. e2e — from a paused sample at 1920 and 390, `Leave sample` returns the page to the first-load screen (strip state declared explicitly, eleven bare rows, "No activity yet", focus on `Play sample session`) and a reload stays there; from a live session with three proposals, `Start over` → `Cancel` changes nothing, `Start over` → confirm clears the page. Screenshots of the replay row in its four states at 1920 and 390 and of the dialog in the pull request.
...
5. `removeModelContext` helper and its use in every existing `no-api` test; strip and log-header entry points incl. the narrow `no-api` exception; orientation strings; `Header`; `SourcePane` tab set from the package; `DrawingSheet` with the whole-sheet box; `FieldList` heading rule; `App` wiring with the announcement → commit.
6. e2e `own-package.spec.ts` (below); fix what it finds → commit. DESIGN.md additions (the row in § Scope, items (a)–(e)) and the `docs/design/README.md` note; screenshots, budgets, acceptance boxes → final commit.
...
- First load (both API states declared): no group heading in the field list; after one `propose_field` through the installed `modelContext`, the headings appear.
