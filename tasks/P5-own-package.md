...
1. **`Leave sample` in the replay row, in every state.** The row's controls become: playing `Pause · Leave sample`; paused `Play · Next step · Leave sample`; ended `Leave sample`; error `Leave sample`. `Restart` is removed everywhere (a 26-step recording is short; watching again is `Leave sample` → `Play sample session`). For an imported session the button reads `Leave session`; the `{label}` rule of P3.1 § Replay controls applies. Behaviour: `leave()` (the P3 detach sequence, one replay owner), then `replaceState(createInitialState())`, the saved session key cleared, the package restored to the sample if this task had switched it, the strip back to `waiting` or `no-api` per the API state, the field list to the eleven bare rows, the change log to "No activity yet", focus on `Play sample session`, live region "Sample session closed" / "Imported session closed". No confirmation: nothing of the person's is lost. The control slot keeps its fixed width (the widest set is the paused one); below 600px the controls stack under the text as P3.1 ruled. The P3.1 rule "`Restart` focuses `Play`" is withdrawn; this section supersedes the controls column of the P3.1 § Replay controls table, and the P3.1 task file gets a one-line pointer to here in the same commit.
