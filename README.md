...
- **ChatGPT desktop app.** Open the URL in the built-in browser in Work mode. As of September 3, 2026 the model has to be GPT-5.6 Sol (Light or Medium) or GPT-5.6 Terra; Luna exposes no site tools, so under Luna the page keeps waiting. The status strip shows the prompt to type: `Extract this RFQ into a quote request`. Fields fill in as the agent reads; the Site tools panel in the address bar lists the page's tools, and its count changes from 6 to 7 when the agent hits a gap and gets the clarification tool.
...
- **Your own package.** `Open your own package` in the status strip: paste the customer email and the specification, attach the drawing as PNG, JPEG or WebP, open, and give your agent the same prompt. The drawing is an image only; an agent that cannot see it reports the drawing number and revision as missing, which is the right answer.
...
- Pasted documents are capped at 40,000 characters each. Longer specifications need trimming before they go in.
- Drawings come in as PNG, JPEG or WebP. PDF sheets need a screenshot first.
- A user package's drawing has no transcribed regions, so nothing on it can be cited. Region transcription, or a read tool that can see the image, is what that path needs next.
